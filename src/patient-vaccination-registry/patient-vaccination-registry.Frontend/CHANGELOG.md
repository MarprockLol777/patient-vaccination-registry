...
- Cree un proyecto de React con create-vite: `npm init --yes vite@latest patient-vaccination-registry.frontend -- --template=react  --no-rolldown --no-immediate`.
- Actualizando `vite.config.js` con puerto.
- Crear archivo de proyecto (`patient-vaccination-registry.frontend.esproj`).
